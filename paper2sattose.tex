...
% Conclusions.
We hope the \texttt{damegender} tool can become a cornerstone for the scientific advancement of the study of gender (including gender gap) and diversity in the IT, and in particular in the free software community.
\end{abstract}
...
problem of name-to-gender inference. Section 3 introduces the Open Datasets used as the \emph{golden truth}.
Section 4 presents a feature comparison with other name-to-gender inference services and tools.
...
Thus, it is possible to download JSON files from the main commercial name-to-gender inference API solutions (e.g., \texttt{genderapi}, \texttt{genderize}, \texttt{namsor}, \texttt{nameapi}) and use it as the dataset.
...
That is why we propose a new tool, called \texttt{damegender}, with the aim of having open data on name-to-gender inference, which offers more flexibility and where researchers can build on top of it.
